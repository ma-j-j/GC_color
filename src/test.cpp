...
void ChipDetect_algo::detecting(Mat& img, int binarynum )
{
	...
	if (img.empty() || img.channels() != 3) {
		cerr << "Invalid input image! (空图像或通道数错误)"
	...
	threshold(img, binaryimg, binarynum, 255, THRESH_BINARY_INV);
	...
	//imshow("testb", temp);
	//imwrite("D:\code\Daheng_code/3.bmp", binaryimg);
	...
	imshow("frame", img);
	imwrite("D:\\code\\Daheng_code\\3.bmp", img);
	waitKey(0);
}
